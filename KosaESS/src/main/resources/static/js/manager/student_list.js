...
function selectClassFill() {
	...
	$('#classId').empty();
	//목록을 채우기
	$('#classId').append('<option value="" disabled selected>교육과정을 선택하세요</option>');
	$.ajax({
		type: 'get',
...
function search() {
	...
	//원본
	var targetClassId = $('#classId option:selected').val();
	if(targetClassId.length >10){
	...
	$('#class_id_save').val(
		targetClassId
		//document.getElementById('classId').options[document.getElementById('classId').selectedIndex].value.split("(")[1].split(",")[0].split("=")[1]
		);
